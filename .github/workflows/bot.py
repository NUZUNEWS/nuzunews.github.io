...
<head>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .video-grid {{
        display: grid;
        /* cols/rows set by packBannerGrid() JS — defaults shown for no-JS */
        grid-template-columns: repeat(5, 1fr);
        grid-template-rows: repeat(2, 1fr);
        gap: 3px; padding: 3px;
        background: #0a0a0a;
        /* Height = window height minus sticky-nav (~48px) and a small gap */
    ...
    .youtube-inset {{
        position: relative;
        overflow: hidden;       /* Clips iframe overscan */
        background: #050505;
        border-radius: 2px;
        /* NO aspect-ratio: cells fill their grid area completely */
    }}
    /* Overscan: same 112%×112% crop technique as WR cells */
    ...
    #wr-grid {{
        ...
        padding: 3px;
        overflow: hidden;
        /* rows/cols set dynamically by packWRGrid() below */
    }}
    .wr-cell {{
        position: relative;
        background: #0a0a0a;
        overflow: hidden;         /* CRITICAL: clips iframe overscan */
        border-radius: 2px;
        cursor: pointer;
        transition: outline 0.15s;
        /* No aspect-ratio constraint — cells fill grid area completely */
    }}
    ...
    .wr-cell iframe {{
        /* Overscan: size 112% × 112% and centre — covers black letterbox bars.
           YouTube embeds have inherent black bars when the player aspect-ratio
           doesn't match the cell. Oversizing + centering crops those bars so
           video fills edge-to-edge. 12% overshoot handles 16:9 content in
           any cell shape without visible stretching. */
        position: absolute;
        top: -6%; left: -6%;
        width: 112%; height: 112%;
        border: none; display: block;
...
(function() {{
  ...
  // ── packWRGrid(): compute optimal cols×rows for the WR overlay viewport ──────
  // Strategy: iterate candidate column counts 2–8. For each, calculate the
  // resulting cell width and height. Choose the layout whose cell aspect ratio
  // is closest to 16:9 AND which maximises the fill ratio (fewest unused cells).
  // Overscan (112% iframe) means the 16:9 target is approximate — being off by
  // 20% is fine because the crop hides the difference.
  function packWRGrid() {{
    var n = WR_FEEDS.length;                       // 10 feeds
    var W = grid.clientWidth  || window.innerWidth;
    var H = grid.clientHeight || (window.innerHeight - 44); // minus header
    if (W < 1 || H < 1) return;
    var GAP = 3;
    ...
    for (var cols = 2; cols <= 8; cols++) {{
      ...
      var targetAR = 16 / 9;
      // Penalise deviation from 16:9 (less important with overscan, so weight low)
      var arPenalty = Math.abs(cellAR - targetAR) / targetAR;
      // Reward fill efficiency: how much of the grid is occupied by real feeds
      var usedCells = n;
      var totalCells = cols * rows;
      var fillRatio  = usedCells / totalCells;
      // Score: maximise fillRatio, minimise arPenalty
      var score = fillRatio * 10 - arPenalty * 2;
  ...
  function _buildWRGrid() {{
    ...
    // Pack grid after cells are in the DOM so clientWidth/clientHeight are valid
  ...
  function openWR() {{
    ...
    // Repack on resize (orientation changes, window resize)
    if (!openWR._resizeHandler) {{
      openWR._resizeHandler = function() {{ if (_wrIsOpen) packWRGrid(); }};
    ...
    try {{
      ...
      if (fsReq && typeof fsReq.then === 'function') {{
        fsReq.then(function() {{ _overlayIsFullscreen = true; requestAnimationFrame(packWRGrid); }}).catch(function() {{}});
      }}
    }} catch(e) {{}}
  }}
  function closeWR() {{
