...
# --- NUZU FIX 1: globally-unique DOM id allocator -------------------------
# The same story legitimately appears in several sections, so an id minted
# from the article URL alone (cl-<md5>, hl-<md5>) was emitted multiple times.
# getElementById only ever returns the FIRST match -> "Go to story" landed on
# the wrong copy. alloc_dom_id() guarantees uniqueness while staying perfectly
# idempotent per (section, base, source-url), so the MRO "Go to story" card and
# the feed element it points at always resolve to the SAME id.
_USED_DOM_IDS = set()
_DOM_ID_BY_KEY = {}   # (ctx, base, src) -> final_id

def alloc_dom_id(base, ctx="", src=None):
    key = (ctx, base, src)
    hit = _DOM_ID_BY_KEY.get(key)
    if hit is not None:
        return hit
    candidate = (base + "-" + ctx) if ctx else base
    final = candidate
    n = 2
    while final in _USED_DOM_IDS:
        final = candidate + "-" + str(n)
        n += 1
    _USED_DOM_IDS.add(final)
    _DOM_ID_BY_KEY[key] = final
    return final
# --------------------------------------------------------------------------

def render_clusters(clusters, show_trust=True, ctx=""):
    now = time.time()
    out = ""
    for cluster in clusters:
        if len(cluster) == 1:
            ...
            anchor_id = alloc_dom_id("hl-" + hashlib.md5(link.encode()).hexdigest()[:8], ctx, src=link)
            ...
            cluster_id = alloc_dom_id("cl-" + hashlib.md5(lead_link.encode()).hexdigest()[:8], ctx, src=lead_link)
...
<head>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .wr-cell iframe {{
        /* JS applies transform:scale() to zoom out black bars — defaults safe */
        position: relative;
...
def render_mro_cards(cards):
    ...
    for section_label, cluster, n_src, lead_ts2, lead_title, lead_link in cards:
        ...
        # NUZU FIX 1: the card's cluster is pre-sorted exactly like the column
        # (newest article = lead), so md5(lead_link) is precisely the hash the
        # column will render. Allocate the unique per-section id from the SAME
        # (ctx, base, src) the feed uses, so the card and its target element
        # always share one id. This is what kills the duplicate-id
        # mis-navigation; the old md5-only ids repeated across sections and
        # getElementById returned whichever copy appeared first.
        _card_sid = section_dom_id.replace("section-", "")
        link_hash = hashlib.md5(lead_link.encode()).hexdigest()[:8]
        anchor_cluster = alloc_dom_id("cl-" + link_hash, _card_sid, src=lead_link) + "-anchor"
        anchor_single  = alloc_dom_id("hl-" + link_hash, _card_sid, src=lead_link)
...
function _nuzu_soft_refresh() {{
  ...
  // NUZU FIX 2: remember the topmost on-screen story so we can re-anchor to it
  // after the live feed swap reorders content. Restoring the raw pixel offset
  // (below) silently landed users on a DIFFERENT story once the order changed.
  var _keepId = null, _keepOffset = 0;
  (function() {{
    var _els = document.querySelectorAll('#sections-wrapper .cluster, #sections-wrapper .headline');
    for (var _i = 0; _i < _els.length; _i++) {{
      var _r = _els[_i].getBoundingClientRect();
      if (_r.bottom > 0 && _els[_i].id) {{ _keepId = _els[_i].id; _keepOffset = _r.top; break; }}
    }}
  }})();
  fetch(window.location.pathname + '?_nc=' + Date.now())
    ...
    .then(function(html) {{
      ...
      document.querySelectorAll('.mro-card').forEach(function(card) {{
        card.addEventListener('click', function() {{
          ...
          window._nz_scroll(target, 'smooth');
      ...
      // NUZU FIX 2: re-anchor to the SAME story element rather than the raw
      // pixel offset, which no longer points at that story after the reorder.
      var _keepEl = _keepId ? document.getElementById(_keepId) : null;
      if (_keepEl) {{
        var _newTop = _keepEl.getBoundingClientRect().top + window.pageYOffset;
        window.scrollTo(0, Math.max(0, _newTop - _keepOffset));
      }} else if (savedScroll < 200) {{
    ...
    .catch(function() {{
      ...
      try {{
        if (_keepId) sessionStorage.setItem('nuzu_soft_anchor', _keepId);
        sessionStorage.setItem('nuzu_soft_scroll', savedScroll);
      }} catch(e) {{}}
      location.reload();
    }});
}}
...
window._nz_scroll = function(el, behavior) {{
  ...
  var _go = function(b) {{
    return function() {{ el.scrollIntoView({{ behavior: b, block: 'start' }}); }};
  }};
  requestAnimationFrame(_go(behavior));
  // NUZU FIX 3: lazy-loaded images and content-visibility:auto change the
  // target's position AFTER the first scroll, leaving the user short of the
  // story ("where it would have been"). Re-align once layout has settled.
  setTimeout(_go('auto'), 350);
  setTimeout(_go('auto'), 900);
}};

// - Restore scroll position if coming from soft-refresh fallback -
(function() {{
  try {{
    var sa = sessionStorage.getItem('nuzu_soft_anchor');
    var ss = sessionStorage.getItem('nuzu_soft_scroll');
    sessionStorage.removeItem('nuzu_soft_anchor');
    sessionStorage.removeItem('nuzu_soft_scroll');
    var saEl = sa ? document.getElementById(sa) : null;
    if (saEl) {{
      saEl.scrollIntoView({{ block: 'start' }});
    }} else if (ss) {{
      window.scrollTo(0, parseInt(ss, 10));
    }}
  }} catch(e) {{}}
}})();
...
// caught by both Tech and Culture keyword filters), FIX 1 now gives each copy a
// globally-unique id (cl-HASH-SID-anchor), so getElementById no longer collides
// across sections. The section-scoped lookup below still runs first as
// belt-and-suspenders, with a full-page fallback so a click never silently fails.
(function() {{
  var NAV_HEIGHT = 60;
  document.querySelectorAll('.mro-card').forEach(function(card) {{
    card.addEventListener('click', function() {{
      ...
      // Route through _nz_scroll so content-visibility + lazy-image settle is handled
      window._nz_scroll(target, 'smooth');
      target.style.transition = 'outline 0s';
      target.style.outline = '2px solid #1E4FD8';
      setTimeout(function() {{ target.style.transition = 'outline 0.8s'; target.style.outline = 'none'; }}, 1400);
...
(function() {{
  ...
  function _applyOverscanToCell(cell) {{
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Instead of repositioning the iframe (which always clips some edge),
    // we keep the iframe at width:100% height:100% and scale the WHOLE
    // player up from its center using CSS transform.
    //
    // YouTube renders its 16:9 video + black bars to fill the iframe.
    // After scale(S), the iframe visually fills S×cw × S×ch, centered on
    // the cell center. The black bars are now S×(bar_px) thick — if S is
    // large enough, they extend beyond the cell's overflow:hidden boundary.
    // The video content and ALL controls scale proportionally and remain
    // centered — volume knob, progress bar, fullscreen button all accessible.
    //
    // Required scale:
    //   Taller cell (cellAR < 16/9): scale = ch / (cw/1.778)
    //                                      = ch * 1.778 / cw
    //   Wider cell  (cellAR > 16/9): scale = cw / (ch*1.778)
    //
    // We cap at 1.5× — beyond this the controls shrink visually too much.
    // Any remaining thin black lines at extreme layouts are acceptable.

    var scale;
    if (cellAR < VIDEO_AR) {{
      scale = (ch * VIDEO_AR) / cw;   // taller cell: scale to fill height
    }} else {{
